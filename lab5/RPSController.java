...
public class RPSController implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		this.view.setComputerLabel(model.getComputerHand());

		if (model.playerWins()) {
			this.view.winner.setText("You win");
		} else if (model.computerWins()) {
			this.view.winner.setText("Computer wins");
		} else if (model.draw()) {
			this.view.winner.setText("It's a draw");
		}

		this.getView().setRecord(model.getRoundsWonByPlayer(), model.getRoundsWonByComputer(), model.getRoundsDrawn());
